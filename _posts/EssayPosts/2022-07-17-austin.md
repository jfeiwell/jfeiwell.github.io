...
Secondly, given the fact early startups don’t need that many people by design, it shouldn’t be hard to hire the first ~20 employees from either the local gene pool or import a few one offs as new Texans. 
...
There are some promising candidates that may serve this role over the next few years. [Royal.io](https://royal.io/) has its roots from the OpenDoor founding team. Bumble is here. [Darwin](www.darwinhomes.com) is built by multiple high level ex-DoorDash. And a handful of other promising companies star in the trailer for our unreleased box office smash. 
